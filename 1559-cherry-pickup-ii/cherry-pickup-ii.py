class Solution:
    ...
    def cherryPickup(self, grid: List[List[int]]) -> int:
        t=[[[None]*(len(grid[0])) for i in range(len(grid[0]))] for j in range(len(grid))]
        n=len(grid)
        #making a possible base case
                ...
                for j2 in range(len(grid[0])):
                    ...
                    for dj1 in range(-1,2):
                        for dj2 in range(-1,2):
                            if j1+dj1>=0 and j1+dj1<len(grid[0]) and j2+dj2>=0 and j2+dj2<len(grid[0]):
                                if j1==j2:
                                    ch=grid[i][j1]+t[i+1][j1+dj1][j2+dj2] 
                                else:
                                    
                                        ch=grid[i][j1]+grid[i][j2]+t[i+1][j1+dj1][j2+dj2]
                            else:
                                ch=float('-inf')
                            ans=max(ans,ch)
                            
                    t[i][j1][j2]=ans
        return t[0][0][len(grid[0])-1]
...
# this is memoiation, still usese more space, need to optimize this more
